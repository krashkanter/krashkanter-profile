...
export default function App({
  ...
  return (
    <div
      ...
      className="flex h-full flex-col items-center justify-between rounded-4xl border-1 border-neutral-300 bg-gradient-to-br from-neutral-100 to-neutral-400 p-4 text-black shadow-2xl shadow-neutral-300"
    >
      <div className="pointer-events-none mb-12 flex h-1/2 w-full flex-col rounded-3xl border-2 border-b-5 border-neutral-600 bg-white p-4 shadow-lg select-none">
        ...
        >
          ...
          {menuItems.map((item, index) => (
            <div
              key={item}
              className={`rounded-md px-2 py-1 text-lg font-medium transition-all duration-200 ease-in-out ${selectedItem === index ? "bg-gradient-to-r from-blue-500 to-blue-400 text-white shadow-md" : "text-neutral-900 hover:bg-gray-800"}`}
            >
              {item}
            </div>
          ))}
